...
@section('page_title', $db->action == 'update' ? __('voyager::database.editing_table', ['table' => $db->table]) : __('voyager::database.create_new_table'))

@section('page_header')
<h1 class="page-title">
    <i class="voyager-data"></i>
    {{ $db->action == 'update' ? __('voyager::database.editing_table', ['table' => $db->table]) : __('voyager::database.create_new_table') }}
</h1>
@stop
...
<script>
    new Vue({
        ...
        data: {
            table: {},
            originalTable: JSON.parse('{!! $db->tableJson !!}'), // to do comparison later?
            oldTable: JSON.parse('{!! $db->oldTable !!}'),
            tableJson: ''
        },
        created() {
            ...
            if (this.oldTable) {
                try {
                    this.table = JSON.parse(this.oldTable);
                } catch (e) {
                    console.error('Error parsing oldTable JSON:', e);
                    this.table = {}; // or handle the error in another way
                }
            } else {
                try {
                    this.table = JSON.parse(JSON.stringify(this.originalTable));
                } catch (e) {
                    console.error('Error parsing originalTable JSON:', e);
                    this.table = {}; // or handle the error in another way
                }
            }
        },
